...
class Contract(sp.Contract):
  def __init__(self):
    ...
    self.init(administrator = sp.address('tz1XTWbfhyWK9xmPAa6TyQUSv437JFgZDzgA'),
  ...
  def setLiquidatePaused(self, params):
    sp.verify(sp.sender == self.data.administrator, 'GOV_NOT_ADMIN')
    sp.set_type(params, sp.TRecord(comptroller = sp.TAddress, tokenState = sp.TRecord(cToken = sp.TAddress, state = sp.TBool).layout(("cToken", "state"))).layout(("comptroller", "tokenState")))
    sp.transfer(params.tokenState, sp.tez(0), sp.contract(sp.TRecord(cToken = sp.TAddress, state = sp.TBool).layout(("cToken", "state")), params.comptroller, entrypoint='setLiquidatePaused').open_some())
  ...
  def setMarketCaps(self, params):
    sp.verify(sp.sender == self.data.administrator, 'GOV_NOT_ADMIN')
    sp.set_type(params, sp.TRecord(caps = sp.TRecord(borrowCap = sp.TNat, cToken = sp.TAddress, supplyCap = sp.TNat).layout(("borrowCap", ("cToken", "supplyCap"))), comptroller = sp.TAddress).layout(("caps", "comptroller")))
    sp.transfer(params.caps, sp.tez(0), sp.contract(sp.TRecord(borrowCap = sp.TNat, cToken = sp.TAddress, supplyCap = sp.TNat).layout(("borrowCap", ("cToken", "supplyCap"))), params.comptroller, entrypoint='setMarketCaps').open_some())
  ...
  def setPriceBounds(self, params):
    sp.verify(sp.sender == self.data.administrator, 'GOV_NOT_ADMIN')
    sp.set_type(params, sp.TRecord(bounds = sp.TRecord(cToken = sp.TAddress, maxChangeBps = sp.TNat, maxPrice = sp.TNat, minPrice = sp.TNat).layout((("cToken", "maxChangeBps"), ("maxPrice", "minPrice"))), comptroller = sp.TAddress).layout(("bounds", "comptroller")))
    sp.transfer(params.bounds, sp.tez(0), sp.contract(sp.TRecord(cToken = sp.TAddress, maxChangeBps = sp.TNat, maxPrice = sp.TNat, minPrice = sp.TNat).layout((("cToken", "maxChangeBps"), ("maxPrice", "minPrice"))), params.comptroller, entrypoint='setPriceBounds').open_some())
  ...
  def setRedeemPaused(self, params):
    sp.verify(sp.sender == self.data.administrator, 'GOV_NOT_ADMIN')
    sp.set_type(params, sp.TRecord(comptroller = sp.TAddress, tokenState = sp.TRecord(cToken = sp.TAddress, state = sp.TBool).layout(("cToken", "state"))).layout(("comptroller", "tokenState")))
    sp.transfer(params.tokenState, sp.tez(0), sp.contract(sp.TRecord(cToken = sp.TAddress, state = sp.TBool).layout(("cToken", "state")), params.comptroller, entrypoint='setRedeemPaused').open_some())
